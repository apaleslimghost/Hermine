					...
					return Array.from(node.connectedEdges()).reduce(
						(colour, edge) =>
							runTypes.includes(edge.data("type"))
								? runTypes[
										Math.min(
											runTypes.indexOf(colour),
											runTypes.indexOf(edge.data("type")),
										)
									]
								: colour,
						"black",
		...
		<ListItem>
			...
			{length && (
				<>
					{" "}
					{length}
					{["green", "blue", "red", "black"].includes(type)
						? "km"
						: " mins"}
				</>
			)}
				...
				<ListItem>
					...
					{exclude.length > 0 && (
						<>
							{" "}
							(excluding{" "}
							{exclude.map((x) => `${getIcon(x)} ${x}`).join(", ")})
						</>
					)}
				</ListItem>
			) : (
...
const App = () => {
	...
	useLayoutEffect(() => {
		...
		cy.edges().forEach((edge) => {
			...
			const d = Math.sqrt(Math.pow(sx - tx, 2) + Math.pow(sy - ty, 2));
		...
		cy.layout({
			...
			stop() {
				cy.elements(
					...
					if (next.length) {
						const { x: nx, y: ny } =
							next[0]._private.target._private.position;

						edge.scratch(
							"_targetDirection",
							Math.abs(sx - nx) > Math.abs(sy - ny)
								? "horizontal"
								: "vertical",
						);
					} else {
					...
					if (prev.length) {
						const { x: px, y: py } =
							prev[0]._private.source._private.position;
						edge.scratch(
							"_sourceDirection",
				...
				cy.elements(
					...
					"segment-weights"(edge) {
						if (
							edge.scratch("_targetDirection") ===
							edge.scratch("_sourceDirection")
						) {
